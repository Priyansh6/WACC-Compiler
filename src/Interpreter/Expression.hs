...
import Error.Semantic (SemanticError (..), arrayErrorType, incompatibleType)
...
len pos v = toWType v >>= semanticError . IncompatibleTypes pos [arrayErrorType]
...
catchOverflow pos val = case val of
  IInt i | i < smallestWaccInt -> runtimeError $ IntegerUnderflow pos
  IInt i | i > biggestWaccInt -> runtimeError $ IntegerOverflow pos
  v -> return v
...
evalUnOp op expr pos expected = do
  ...
  case eType of
    ...
    wt -> semanticError $ IncompatibleTypes pos [expected] wt
...
evalNumericBinOp' op exp1 exp2 pos = do
  ...
  if divisor == IInt 0
    then runtimeError $ DivideByZero pos
...
isValidNumericOperator pos e1 e2 = do
  e1Type <- toWType e1
  e2Type <- toWType e2
  semanticError (incompatibleType pos [WInt] e1Type e2Type) >> return False
...
isValidOrdOperator pos e1 e2 = do
  e1Type <- toWType e1
  e2Type <- toWType e2
  semanticError (incompatibleType pos [WInt, WChar] e1Type e2Type) >> return False
...
isValidBoolOperator pos e1 e2 = do
  e1Type <- toWType e1
  e2Type <- toWType e2
  semanticError (incompatibleType pos [WBool] e1Type e2Type) >> return False
...
getArrayElem (ArrayElem ident indices pos) =
  getVarOrParam ident
    >>= \case
      ...
      _ -> semanticError $ NonSubscriptable pos
  where
    ...
    getArrElem (IArr addr') ((IInt i) : is) = do
      ...
      if null arr
        then runtimeError $ IndexOutOfBounds pos
        else
          if 0 <= i && i <= toInteger (length arr) - 1
            then getArrElem (arr !! fromInteger i) is
            else runtimeError $ IndexOutOfBounds pos
    getArrElem (IArr _) (i : _) = toWType i >>= semanticError . IncompatibleTypes pos [WInt]
    getArrElem _ _ = semanticError $ NonSubscriptable pos
