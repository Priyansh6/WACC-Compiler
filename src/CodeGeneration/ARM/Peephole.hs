...
module CodeGeneration.ARM.Peephole (peepholeOptimise) where 

import CodeGeneration.Intermediate.IR
import CodeGeneration.ARM.Registers
import Data.Maybe ()

peepholeOptimise :: Program ArmReg -> Program ArmReg
peepholeOptimise = map optimiseSection 
...
optimiseInstrs (m@(Mov _ (Imm _)) : p@(Push _) : instrs) = m : optimiseInstrs (p : instrs)
optimiseInstrs (m@(Mov o1@(Reg r) (Reg r1)) : p@(Push (Regs [r2])) : instrs) 
...
optimiseInstrs (m@(Mov _ (Imm _)) : s@(Store _ _) : instrs) = m : optimiseInstrs (s : instrs)
...
optimiseInstrs (l@(Load o1 o2) : m@(Mov o3 o4) : instrs)
    | o1 == o4 && isScratchOperand o1 = optimiseInstrs ((Load o3 o2) : instrs)
    | otherwise = l : optimiseInstrs (m : instrs)
