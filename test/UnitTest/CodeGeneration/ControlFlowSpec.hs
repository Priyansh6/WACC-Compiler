...
spec = do
  ...
  it "constructs a singleton node for a body of one instruction" $ 
    (nodes cfg1) ! (Right 0) `shouldBe` mkCFGNode (Right 0) (Mov (Reg (TmpReg 1)) (Imm 8)) 
  ...
  it "constructs nodes with a label id for define instructions" $ 
    (nodes cfg2) ! (Left "label") `shouldBe` mkCFGNode (Left "label") (Define "label") 
  ...
  it "constructs multiple nodes for a body of multiple instructions" $ 
    nodes cfg3 `shouldBe` M.fromList [ (Right 0, mkCFGNode (Right 0) (Mov (Reg (TmpReg 1)) (Imm 8)) ) 
                                     , (Right 1, mkCFGNode (Right 1) (Mov (Reg (TmpReg 2)) (Imm 7)) )
                                     , (Right 2, mkCFGNode (Right 2) (Add (Reg (TmpReg 1)) (Reg (TmpReg 2)) (Reg (TmpReg 1))) ) ]
