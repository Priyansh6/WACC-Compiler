...
spec = do
  ...
  it "constructs a singleton node for a body of one instruction" $ 
    (nodes cfg1) ! (Right 0) `shouldBe` mkCFGNode (Mov (Reg (TmpReg 1)) (Imm 8)) True
  ...
  it "constructs nodes with a label id for define instructions" $ 
    (nodes cfg2) ! (Left "label") `shouldBe` mkCFGNode (Define "label") True
  ...
  it "constructs multiple nodes for a body of multiple instructions" $ 
    nodes cfg3 `shouldBe` M.fromList [ (Right 0, mkCFGNode (Mov (Reg (TmpReg 1)) (Imm 8)) True) 
                                     , (Right 1, mkCFGNode (Mov (Reg (TmpReg 2)) (Imm 7)) False)
                                     , (Right 2, mkCFGNode (Add (Reg (TmpReg 1)) (Reg (TmpReg 2)) (Reg (TmpReg 1))) False) ]
